    ...
    private String createClientDataJson(PublicKeyCredentialRequestOptions requestOptions, String originalJson) {
        ...
        try {
            ...
            if (originalJsonNode.has("extensions") && 
                originalJsonNode.get("extensions").has("payment")) {
                
                JsonNode paymentNode = originalJsonNode.get("extensions").get("payment");
                
                if (paymentNode.has("isPayment") && paymentNode.get("isPayment").asBoolean()) {
                    log.debug("SPC payment extension detected, adding payment field to clientDataJSON");
                    clientData.set("payment", paymentNode);
                }
            }
        } catch (Exception e) {
            log.debug("Could not process payment extension: {}", e.getMessage());
        }
